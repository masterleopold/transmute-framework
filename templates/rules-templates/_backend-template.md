...
> **This is a template.** Stage 3 (Scaffold Generation) reads this template and generates `.claude/rules/backend.md` with actual project values. Stage 3 MUST: (1) replace ALL `[BRACKETED]` placeholder markers (e.g., `[BACKEND_DIR]`, `[BACKEND_FRAMEWORK]`, `[ERROR_TYPE]` (cross-template: must match `.claude/rules/auth.md`), `[AUTH_HELPER]` (cross-template: must match `.claude/rules/auth.md`), `[VALIDATOR_SYSTEM]`, `[SOFT_DELETE_FIELD]` (cross-template: must match `.claude/rules/data-model.md`), `[TENANT_ID_FIELD]` (cross-template: must match `.claude/rules/data-model.md`)), (2) replace each `<!-- TODO -->` HTML comment with a proper `<!-- Source: Stage 3 | Evidence: [ref] | Confidence: HIGH -->` annotation (`// TODO:` inside code blocks are code example placeholders — replace those with actual code patterns), (3) update the globs in frontmatter with actual paths, and (4) remove ALL other HTML comments (e.g., `<!-- Stage 3: ... -->`, `<!-- Glob note: ... -->`, `<!-- Note: ... -->`) — these are template-only guidance that must not appear in generated rule files. Stage 4 confirms replacements are complete. After Stage 3 renders this template, verify no placeholders remain: `grep -nE '\[[A-Z_]+\]' .claude/rules/backend.md` — the output should be empty (all `[BRACKETED]` markers replaced with actual values). **Rule count limit**: The rendered output must contain ≤ 15 rules (individual bullet-point directives). This template contains ~15 baseline rules — Stage 3 has room for at most 2 additional stack-specific rules before splitting is required. This template contains conditional sections — omit sections that don't apply to the selected tech stack. If the rendered output exceeds 15 rules after omitting inapplicable sections, split into two rule files (e.g., `backend.md` → `backend.md` + `backend-queries.md`) and update CLAUDE.md Part 2 § Path-Scoped Rules accordingly. Do not edit this template directly — edit the generated `.claude/rules/backend.md` instead.

## Validation

<!-- TODO: Stage 3 — replace with actual validation pattern for [BACKEND_FRAMEWORK]. Source: tech-stack.md | Confidence: HIGH -->

<!-- Valid values: e.g., "Zod", "Valibot", "io-ts", "Convex v.object()", "Drizzle createInsertSchema()" -->
...
<!-- If backend uses a different config location (e.g., Convex uses convex.json for some settings), document the actual location here -->
...
- **Credential verification**: After setting auth provider credentials (API keys, client IDs, client secrets) in environment variables, verify them by making a lightweight API call (e.g., list users, get account info) to confirm the credentials are valid and match the intended environment (staging vs production). Character-level typos in credential values (e.g., `Q` vs `0`) cause cryptic "invalid client" errors that are difficult to diagnose.
