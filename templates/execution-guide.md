...
1. Exit the Claude Code session after reaching the session feature limit
2. Start a fresh session with `claude --dangerously-skip-permissions`
3. Paste the Stage 5 orchestrator prompt again
4. It will read `_progress.md`, see which features are ✅ Done, and resume from the next incomplete feature

For features that span 15+ files, split them into sub-features (FEAT-003a, FEAT-003b) in `plancasting/_progress.md` before starting Stage 5.
...
**Ordering note**: Optimal after 6G (all code changes finalized) per the recommended ordering above. Can run after 5B PASS or CONDITIONAL PASS for an initial draft, but may need updates after 6E–6G modify code.
...
For **migration-file workflows** (Drizzle, Prisma, Knex, raw SQL): promote schema changes following the schema promotion order in `plancasting/tech-stack.md` § "Database Migration Strategy".
...
**Prerequisite**: Stage 7V PASS (product deployed and verified in production).
...
Create `./feedback/input.md` with structured user feedback collected since the last cycle. Include support tickets, analytics data, survey results, and stakeholder requests. Follow the format specified in the prompt.
...
**Stage 2B**: PASS / CONDITIONAL PASS / FAIL — coverage-based with P0 coverage as binding constraint. PASS requires 0 CRITICAL, 0 HIGH, ≥95% overall coverage. CONDITIONAL PASS allows ≤3 HIGH (no P0-blockers), ≥90% overall coverage. FAIL if any unresolved CRITICAL, P0 coverage <95%, or overall coverage <90%. Full decision tree with 8 rules in `prompt_validate_specs.md`.

**Stage 3**: PASS (≥95% scaffold coverage), CONDITIONAL PASS (≥80%), FAIL (<80%) — thresholds defined in the Stage 3 prompt (`prompt_generate_scaffolding.md`).

**Stage 5B gate** (4 outcomes):

| 5B Result | Category C Count | Category A/B Count | Next Step |
|---|---|---|---|
| PASS | 0 | 0 | → Stage 6 (all audits) |
| CONDITIONAL PASS | ≤3 (each with documented workaround + explicit deferral) | ≤2 (documented with explanation) | → Stage 6 (known issues tracked in report) |
