...
- **BRD Accessibility Requirements**: Search with `grep -rl 'accessibility\|WCAG\|a11y' ./plancasting/brd/` to find the relevant BRD files. Common locations: `08-non-functional-requirements.md`, `11-user-experience-requirements.md` (file numbers may vary by project). Note: BRD file numbering is project-generated by Stage 1 and may differ across projects — always use the grep search to locate the correct file.
- **BRD UX Requirements**: Search with: `grep -rl 'user experience\|UX' ./plancasting/brd/`. Common location: `11-user-experience-requirements.md` (file number may vary by project).
