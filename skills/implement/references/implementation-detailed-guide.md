...
## Startup: Reading `_codegen-context.md`

Before feature analysis, read `./plancasting/_codegen-context.md` (generated by Stage 3). This file contains naming conventions, patterns, and scaffold structure established during code generation. It tells the implementation teammates:
- How files are named and organized
- What patterns to follow for consistency
- Which scaffold files exist and their purpose

If this file is missing, fall back to manual directory scanning and pattern inference from existing code.
...
### Teammate Completion Message Format

When done, message the lead with:
- **Files created/modified** (with full paths)
- **Exported symbols** (function names, their types: query/mutation/action/internal)
- **Schema/data changes** (new tables, new indexes, modified tables — specify what changed)
- **Assumptions made**
- **Test results** (new tests + regression check)
- **Integration notes for the next teammate** (what the frontend needs to know: new hooks available, data shapes, auth requirements)
...
When done, message the lead with:
- **Files created/modified** (with full paths)
- **Exported symbols** (component names, hook names)
- **Schema/data changes** (if any frontend types changed)
...
0. **SCAFFOLD INVENTORY**: Check `e2e/` for existing scaffold test files for this feature. If scaffold E2E files exist (from Stage 3), implement inside them rather than creating new files.
...
**Test data cleanup**: Ensure tests clean up created data. Use `afterEach`/`afterAll` hooks. Document setup requirements.
...
- **Integration notes for the next teammate** (any bugs discovered, PRD gaps found, flaky test patterns)
...
2. **Verify full integration**: Run typecheck, all unit/integration tests, all E2E tests.
3. **Fix regressions**: Diagnose and fix. Spawn fix teammate if needed.
4. **Cross-feature verification**: If this feature modified files belonging to other features, verify those features still work.
5. **Module map sync check** (if applicable): Verify new backend function files are registered in the test module map.
6. **Traceability check**: Every user story has backend + frontend + test coverage.
7. **Design consistency check**: Verify tokens, aesthetic direction, visual consistency.
8. **Scaffold manifest update**: If NEW files were created, update `plancasting/_scaffold-manifest.md`.
9. **Stub scan**: Run Anti-Stub Quality Gates grep scan. If any stubs found, send back to frontend teammate.
10. **Update progress**: Mark feature as Done in `plancasting/_progress.md`, record cross-feature mods, assumptions, PRD gaps.
11. **Shutdown teammates** for this feature cycle.
12. **Proceed to next feature** — do NOT stop at priority boundaries.
...
- **Dependency conflicts**: Mark as `⏸ Blocked` in `plancasting/_progress.md` with the blocking feature noted in Notes. Continue with non-blocked features. After each completion, scan for unblocked features.
...
1. **No placeholder text**: Grep for `implementation pending`, `pending feature build`, `⚠️ STUB`, `TODO [Stage 5]`, `Coming soon`, `Not yet implemented`, `PLACEHOLDER`. Zero matches. (`Coming soon` is only acceptable as a user-facing UI label in components that the PRD explicitly defines as showing a "coming soon" state.)
...
If typecheck/lint errors are introduced by THIS feature's new code, fix them before marking done. If errors are PRE-EXISTING, document in the feature brief notes and proceed.
...
Priority order for fixes and escalations:
...
Explicit thresholds:
- **Critical blockers requiring immediate fix**: (1) any page load >10s on 4G throttle, (2) bundle >100% over stated budget. Flag to lead immediately.
...
2. In Progress: check `plancasting/_briefs/` for where implementation stopped. Inspect codebase to establish sub-status: (a) Backend complete? (b) Frontend complete? (c) Tests complete? Resume from first incomplete layer.
...
11. ALWAYS run Full-Product Completion Sequence before final report. The sequence MUST run at least once, even if interrupted previously. If `plancasting/_implementation-report.md` exists but lacks a "Launch Readiness Assessment" section, re-run the Full-Product Completion Sequence.
...
1. **Stub pattern**: Frontend components remain as scaffolds with placeholder text (~80% of issues). Caused by quality gate relaxation in long implementation sessions.
2. **Duplication pattern**: Frontend teammate builds UI inline in page files instead of inside scaffold component files, creating orphan components. Caused by not inventorying scaffold files before writing code.

The mandatory SCAFFOLD INVENTORY step is the PRIMARY defense. If context window is becoming saturated late in a long feature queue, end the session and resume fresh rather than degrading quality. Prioritize BACKEND completeness — backend stubs are harder to fix in 5B.
