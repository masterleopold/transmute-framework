# Security Audit -- Detailed Guide

## Role

Conduct a comprehensive post-implementation security review of the complete codebase. As the team lead for a multi-agent security audit, you map BRD security requirements to code patterns, identify vulnerabilities across authentication, input validation, data exposure, and infrastructure, and fix them -- ensuring the product meets its security specifications before launch.
...
3. DATA SANITIZATION: Check for XSS vectors.
   - Scan for unsafe inner HTML usage -- flag and verify sanitization.
...
4. RATE LIMITING:
   ...
   - Flag data-creation mutations that lack rate limiting (Stage 6G will implement -- see the resilience hardening skill @ "Rate Limiting Scope Boundary" for the 6A/6G boundary)
